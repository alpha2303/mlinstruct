...
class Evaluator:
    ...
    def generate_roc_curve(self, Y_truth_tensor: torch.Tensor, y_pred: torch.Tensor):
        ...
        """
        fpr, tpr, thresholds = roc_curve(Y_truth_tensor.cpu().numpy(), y_pred.detach().cpu().numpy())
        roc_auc = auc(fpr, tpr)
    ...
    def generate_confusion_matrix(self, Y_truth_tensor: torch.Tensor, y_pred: torch.Tensor, threshold: float = 0.5):
        """
        Generate and save plot of ROC Curve obtained by comparing ground truth and predicted target values.
        ...
        plt.annotate(text="Accuracy: %0.2f" % accuracy, xy=(40, 13), xycoords="figure pixels")
