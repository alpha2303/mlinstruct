...
class TestConfusionMatrix(TestCase):
    ...
    def setUpClass(cls) -> None:
        ...
        cls.y_pred_invalid_values: np.ndarray = np.array(
            [-1, 1, 1, 1, 1, 2, 2, 1, 2, 0, 2, 0, 1, 0, 2, 0, 1, 1, 0, 0, 2, 2, 0, 0, 1, 2, 2, 2, 0, 1, 2, 0, 2, 2, 0, 1, 2, 2, 2, 0, 2, 1, 0, 0, 2, 0, 1, 1, 0, 2]
        )
...
def test_matches_sklearn_confusion_matrix(seed: int) -> None:
    ...
    assert np.array_equal(
        cm.as_ndarray(), sklearn_confusion_matrix(y, y_pred, labels=range(4))
    )
